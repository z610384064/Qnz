...
public class ADCircleMonthView extends MonthView {
    ...
    protected void drawBG(Canvas canvas, int column, int row, int day) {
       ...
       // if (day == selDay && !mToday) { //绘制背景色圆形
            paint.setColor(Color.parseColor("#76adf3"));
            paint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(cx, cy, radius * 3 / 2, paint);
        }
        if (day == currDay && currDay != selDay && currMonth == selMonth) {//今日绘制圆
            paint.setColor(Color.parseColor("#fc7946"));
            paint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(cx, cy, radius * 3 / 2, paint);
        }
    }

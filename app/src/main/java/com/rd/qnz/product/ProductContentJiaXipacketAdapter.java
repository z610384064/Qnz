...
public class ProductContentJiaXipacketAdapter extends BaseAdapter {
    ...
    public ProductContentJiaXipacketAdapter(Context context, List<Map<String, String>> list, int buy_money, int timeLimitDay,float extraAwardApr) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = mInflater.inflate(R.layout.homepage_product_jiaxipacket_item, null);


            holder.jiaxi_relativeLayout = (LinearLayout) convertView.findViewById(R.id.jiaxi_relativeLayout); //整个布局
            ...
            holder.redpacket_money_tv = (TextView) convertView.findViewById(R.id.redpacket_money_tv); //百分号
            ...
            holder.redpacket_icon = (ImageView) convertView.findViewById(R.id.redpacket_icon); //打钩的图片
        ...
        if (list.size() > 0) {
            ...
            //holder.redpacket_title.setText(AppTool.getInformationTwoDateDistanceDay(list.get(position).get("validDate")));
            ...
            if (days==0){
//                holder.tv_jiaxi_days.setText("• 加息时长:无限制");
        ...
        if (extraAwardApr!=0){  //是加息标,那么加息劵肯定不可用
            holder.redpacket_icon.setVisibility(View.GONE);
            ...
            if (timeMax == 0 && timeMin == 0) { //代表没有时间限制,那就判断金额
                ...
                if (amountMin==0&&amountMax==0){ //代表金额也没有限制
                    ...
                    if (list.get(position).get("status").equals("1")) {
                        holder.redpacket_icon.setBackgroundResource(R.drawable.selected_btn_ok); //打钩
                    } else {
                        holder.redpacket_icon.setBackgroundResource(R.drawable.selected_btn_def);//不打勾
                    }
                    holder.jiaxi_relativeLayout.setBackgroundResource(R.drawable.jiaxi_image);
    ...
    private class ViewHolder {
        LinearLayout jiaxi_relativeLayout; //整个布局
        TextView tv_jiaxi_name; //加息劵名称

        TextView tv_jiaxi_apr; //加息比例
        TextView redpacket_money_tv; //百分号

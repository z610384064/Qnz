...
/**
 * 可用红包,不可用红包都是这个adapter
 */
public class ProductContentRedpacketAdapter extends BaseAdapter {
    ...
    public ProductContentRedpacketAdapter(Context context, List<Map<String, String>> list, int buy_money, int timeLimitDay) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = mInflater.inflate(R.layout.homepage_product_redpacket_item, null);
        ...
        if (list.size() > 0) {
            ...
            if (timeLimitDay < Integer.parseInt(list.get(position).get("timeLimit"))
                ...
                if (list.get(position).get("status").equals("1")) {
                    holder.redpacket_icon.setBackgroundResource(R.drawable.selected_btn_ok);
                } else {
                    holder.redpacket_icon.setBackgroundResource(R.drawable.selected_btn_def);
                }

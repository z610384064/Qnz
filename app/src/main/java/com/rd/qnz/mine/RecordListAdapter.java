...
public class RecordListAdapter extends BaseAdapter {
    ...
    public RecordListAdapter(Context context, List<RecordItem> list, int type) {
        this.context = context;
        this.list = list;
        this.type = type;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (list.size() > 0) {
            ...
            holder.product_time.setText(AppTool.getMsgTwoDateDistance_miao(list.get(position).getTenderTime()));//投资时间
            holder.product_rate.setText("¥ "+df.format(Math.abs(Double.parseDouble(list.get(position).getInterest()))));//收益

            holder.product_money.setText("¥ "+df.format(Double.parseDouble(list.get(position).getCaptial())));  //本金
            holder.product_return_time.setText(AppTool.getMsgTwoDateDistance1(list.get(position).getRepaymenTime()));

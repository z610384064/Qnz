 ...
 */
public class Assetsincome extends BaseActivity {
    ...
    ** 我的资产界面
    ...
    private TextView first_lines, second_lines, third_lines, fourth_lines,
            first_text, second_text, third_text, fourth_text,
            income_money,
            income_name;
    private TextView five_lines,five_text ;//待收加息描述/待收加息文本
    private TextView six_lines,six_text ;//已收加息描述/已收加息文本
    private RelativeLayout org_rel;  //提现支付中
    /**
     *  额外加息
     */
    private RelativeLayout relativeLayout5; //待收额外加息
    private RelativeLayout relativeLayout6; //已收额外加息
    private LinearLayout linearLayout5,linearLayout6;


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.assets_income);
        type = getIntent().getIntExtra("type", 1);
        investingWaitInterest = getIntent().getDoubleExtra("investingWaitInterest", 0); //待收利息
        if (type == 1) {  //对应的是我的资产界面
        ...
        if (type == 1) {
            ...
            income_money.setText(formatMoney(sum));
            if (sum != 0) {
//                PieData mPieData = getPieData(4, 100, (float) (received / sum), (float) (investingWaitInterest / sum), (float) (useRedpacketSum / sum), 0);//已收、待收、红包
    ...
    private void initBar() {
        ...
        if (type == 1) {
            actionbar_side_name.setText("我的资产");
        } else if (type == 2) {
    ...
    public void intView() {
        ...
        TextView line_tv = (TextView) findViewById(R.id.line_tv);
        income_money = (TextView) findViewById(R.id.income_money);
        income_name = (TextView) findViewById(R.id.income_name);
        first_lines = (TextView) findViewById(R.id.first_lines);
        second_lines = (TextView) findViewById(R.id.second_lines);
        third_lines = (TextView) findViewById(R.id.third_lines);
        fourth_lines = (TextView) findViewById(R.id.fourth_lines);
        ...
        five_lines= (TextView) findViewById(R.id.five_lines);
        five_text= (TextView) findViewById(R.id.five_text);
        six_lines= (TextView) findViewById(R.id.six_lines);
        six_text= (TextView) findViewById(R.id.six_text);
        if (type == 1) { //我的资产界面
            org_rel.setVisibility(View.VISIBLE);
...
//            relativeLayout6.setVisibility(View.GONE);
            ...
            first_lines.setText("可用余额");
            second_lines.setText("待收本金");
            third_lines.setText("待收利息");
            fourth_lines.setText("提现支付中");
        ...
        } else if (type == 2) {  //我的收益界面
            org_rel.setVisibility(View.GONE);
...
//            relativeLayout6.setVisibility(View.VISIBLE);
            ...
            first_lines.setText("已收基础利息");
            second_lines.setText("待收基础利息");
            third_lines.setText("已用红包");
            five_lines.setText("待收额外利息");
            six_lines.setText("已收额外利息");
